...
export default function Resultados() {
  ...
  function goContacto() {
    ...
    if (total <= 2) {
      ...
      setRecomendacion("Se encuentra en los parametros normales");
    } else if (total >= 3 && total <= 5) {
      setFactorReflejos("Reflejos: " + total);
      setRecomendacionReflejos("Se detecto un retraso leve");
    } else if (total >= 6 && total <= 9){
      setFactorReflejos("Reflejos: " + total);
      setRecomendacionReflejos("Se detecto un retraso moderado");
    } else {
      setFactorReflejos("Reflejos: " + total);
      setRecomendacionReflejos("Se detecto un retraso severo");
    }
  }
  ...
  useEffect(() => {
    ...
    const obtenerReflejos = async () => {
      try {
        ...
        setReflejosData([
          { id: 0, value: arcaicos.data ?? 0, label: "Arcaicos", color: "#4caf50" },
          { id: 1, value: primerNivel.data ?? 0, label: "1er nivel", color: "#2196f3" },
          { id: 2, value: segundoNivel.data ?? 0, label: "2do nivel", color: "#9c27b0" },
        ]);
      } catch (error) {
      ...
      <div className={styles.mainContainer}>
        <div className={styles.pieContainer}>
          <div className={styles.infoContainer}>
            <p>PDN: Posible daño neurológico</p>
            <p>ARDN: Alto riesgo de daño neurológico</p>
            <p>PRND: Presunción de daño neurológico definitivo</p>
          </div>
